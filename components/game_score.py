...
from components.const import MAX_LIVES, GOLD


class GameScore:
    def __init__(self, game_play):
        self.game_play = game_play
        self.health_bar = HealthBar(bar_color=GOLD, roundness=.2, max_value=MAX_LIVES)
        self.health_bar.value = self.game_play.game_stats.lives

        self.score = Text(text='Score: 0', color=GOLD)
